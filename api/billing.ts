...
import type { BillingRecord } from '../src/types';
...
export default async function handler(req: Req, res: Res) {
  try {
    ...
    switch (req.method) {
      case 'GET':
        return ok(res, await repo.listBilling(userId));
      case 'PUT':
        return ok(res, await repo.upsertBilling(userId, readBody<BillingRecord>(req)));
      case 'DELETE': {
        const id = readQueryId(req);
        if (!id) return fail(res, 400, 'VALIDATION_ERROR', 'Missing id.');
        await repo.deleteBilling(userId, id);
