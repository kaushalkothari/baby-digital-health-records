...
import type { CreateDocumentInput } from '../src/lib/api/contracts';
...
export default async function handler(req: Req, res: Res) {
  try {
    ...
    switch (req.method) {
      case 'GET':
        return ok(res, await repo.listDocuments(userId));
      case 'POST': {
        const body = readBody<CreateDocumentInput>(req);
        return ok(
          res,
          await repo.createDocument(userId, body.document, body.dataUrl, body.fileType),
          201,
        );
      }
      case 'DELETE': {
        ...
        await repo.deleteDocument(userId, id);
