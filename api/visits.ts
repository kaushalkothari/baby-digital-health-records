...
import type { HospitalVisit } from '../src/types';
...
export default async function handler(req: Req, res: Res) {
  try {
    ...
    switch (req.method) {
      case 'GET':
        return ok(res, await repo.listVisits(userId));
      case 'PUT':
        return ok(res, await repo.upsertVisit(userId, readBody<HospitalVisit>(req)));
      case 'DELETE': {
        const id = readQueryId(req);
        if (!id) return fail(res, 400, 'VALIDATION_ERROR', 'Missing id.');
        await repo.deleteVisit(userId, id);
